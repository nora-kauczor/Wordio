...
class CheckServiceTest {
    ...
    @Test
    void isUserAnswerCorrect_ShouldReturnTrue_whenCalledWithIdOfUneProfesseureAndUserAnswerUnProfesseur() {
        Vocab testVocab = new Vocab("vocab id", "un(e) professeur(e)", "a teacher",
                "", Language.FRENCH, new HashMap<>(), "Wordio");
        String userAnswer = " un professeur";
    ...
    @Test
    void isUserAnswerCorrect_ShouldReturnFalse_whenCalledWithIdOfUneProfesseureAndUserAnswerUnProfesseure() {
        Vocab testVocab = new Vocab("vocab id", "un(e) professeur(e)", "a teacher",
                "", Language.FRENCH, new HashMap<>(), "Wordio");
        String userAnswer = " un professeure";
        when(mockVocabRepo.findById("vocab id")).thenReturn(Optional.of(testVocab));
        assertFalse(checkService.isUserAnswerCorrect("vocab id", userAnswer));
    }

    @Test
    void isUserAnswerCorrect_ShouldReturnTrue_whenCalledWithIdOfUneTableAndUserAnswerUneTable() {
        Vocab testVocab = new Vocab("vocab id", "une table", "a table",
                "", Language.FRENCH, new HashMap<>(), "Wordio");
        String userAnswer = "une table";
        when(mockVocabRepo.findById("vocab id")).thenReturn(Optional.of(testVocab));
        assertTrue(checkService.isUserAnswerCorrect("vocab id", userAnswer));
    }
